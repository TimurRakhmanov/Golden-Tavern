...
func (m *postgresDBRepo) InsertRoomRestriction(r models.RoomRestriction) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt,
		...
		r.ReservationID, time.Now(), time.Now(),
		r.RestrictionID)
	if err != nil {
		return err
	}
...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	...
	password, access_level, created_at, updated_at where id = $1`
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `
	select 
		r.id, r.first_name, r.last_name, r.email, r.phone, r
		.start_date, r.end_date, r.room_id, r.created_at, r.updated_at, r.processed
		rm.id, rm.room_name 
	from 
		reservations r left join rooms rm on (r.room_id = rm.id)
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&i.ID, &i.FirstName, &i.LastName, &i.Email, &i.Phone, &i.StartDate,
			&i.EndDate, &i.RoomID, &i.CreatedAt, &i.UpdatedAt, &i.Processed,
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	select 
		r.id, r.first_name, r.last_name, r.email, r.phone, r
		.start_date, r.end_date, r.room_id, r.created_at, r.updated_at, 
		rm.id, rm.room_name 
	from 
		reservations r left join rooms rm on (r.room_id = rm.id)
	where 
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&i.ID, &i.FirstName, &i.LastName, &i.Email, &i.Phone, &i.StartDate,
			&i.EndDate, &i.RoomID, &i.CreatedAt, &i.UpdatedAt,
			&i.Room.ID, &i.Room.RoomName,
		)
		if err != nil {
			return reservations, err
